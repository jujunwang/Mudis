...
项目介绍：Mudis是一个用go语言实现的Redis服务端程序，区别于Redis的单线程存储引擎，Mudis的存储引擎是并行的，并且在多线程的前提下，能够保证提供的各种操作是线程安全的，因为充分利用了多核性能，有效地减少了处理大key造成主线程阻塞的时间。
...
**Mudis的文件事件分派器不需要等待一个IO事件处理完成，换句话说IO事件的处理是并行进行的，而事件处理完成的快速响应则是依靠netpoller的IO多路复用机制来实现。**
